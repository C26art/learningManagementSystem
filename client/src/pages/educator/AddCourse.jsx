...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      if (!image) {
        toast.error('Thumbnail not select');
      }
      const courseData = {
        courseTitle,
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between p-4 md:p-8 md:pb-0 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex md:flax-row flex-col items-center gap-3'>
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  <img src={assets.dropdown_icon} width={14} onClick={() => handleChapter('toggle', chapter.chapterId)} alt="" className={`mr-2 cursor-pointer transition-transform duration-300 ${chapter.collapsed && "-rotate-90"}`} />
